...
Status: v0.99.9, live on two production OWBN chronicles (kony-sabbat.net, Boston By Night). Working through a defined set of remaining items toward a 1.0 release.
...
Toward 1.0: an 11-item closed scope, each already given a full design pass. Four are now complete (chronicle rename; the read-only sheet's phone-width fix; the background-use ledger and its settings; query beyond characters). What remains: a signed/verifiable PDF plus the reports/cards/batch-output layer that shares its generator, a point calculator, a verification endpoint and chronicle-to-chronicle transfer plus a binary export option (XML export itself already shipped), the rest of the mobile-first sheet work, guided chronicle setup, expanding the Mage rotes catalog from the published Grimoire compendium, and bylaw-driven approval data pending a review-workflow decision.
