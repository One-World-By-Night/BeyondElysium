...
- **Close** - puts the history away.
- The list itself, newest submission first. Each entry shows:
  ...
  - A **Catalog update** line, one per character, when a site is switched to the newer catalog. Open **Matched entries** under it to see each custom entry that became a catalog item, as "what it was → what it is". See [Switching a Site to the Declared Catalog](catalog-cutover.md).
  ...
  - The Storyteller's note, labeled "Storyteller: ...," if there is one - kept separate from
    the submitter's note, never replacing it.
- "No changes yet." when the character has no history at all.

## Common tasks

### Check whether a change is still pending
...
1. Open **View history**.
2. Look under the change for a line starting "Storyteller:".

### Read history alongside a printed or on-screen sheet

1. Pick **Print / Export**, click **Go**, and check **XP History**.
