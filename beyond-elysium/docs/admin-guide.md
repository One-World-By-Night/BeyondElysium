...
Beyond Elysium ships two catalogs. The old one shares Abilities, Merits, Flaws and Werewolf rites across every creature type. The newer one gives each creature type its own lists, prices and groupings. **A new install starts on the newer one**, so none of this applies to it. A site set up before 1.3.4 stays on the old one until someone switches it, once, with WP-CLI, and it can change back.
