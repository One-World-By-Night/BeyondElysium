...
class Game_Import_Controller extends Base_Controller {
	...
	/**
	 * Fetches a previously parsed job's preview, optionally against a candidate merge target given by
	 * `?target=<game_slug>`.
	 *
	 * @param \WP_REST_Request $request
	 * @return \WP_REST_Response|\WP_Error
	...
	/**
	 * Applies a previously parsed and reviewed game-file import job, re-deriving the preview fresh from the stored job;
	 * refuses to commit while anything is unresolved or an unaddressed duplicate remains.
	 *
	 * @param \WP_REST_Request $request
	 * @return \WP_REST_Response|\WP_Error
	 */
	...
	private function commit_locked( $request, string $job_key ) {
		...
		// Fires be_after_upgrade for a new chronicle once it has committed.
