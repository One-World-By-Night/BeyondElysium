...
export function AdminAprSettings() {
	...
	useEffect( () => {
		...
		Promise.all( [
			...
			api.apr( gameSlug ).backgroundOptions(),
		] )
			.then( ( [ found, opts ] ) => {
				setSettings( found );
				setOptions( opts );
	...
	function restoreGrapevineDefaults() {
		...
		setSettings( ( prev ) =>
			prev ? { ...prev, ...GV_DEFAULTS, ...rumorTogglesOf( prev ) } : prev
		);
	}
	...
	function addLevel() {
		const level = window.prompt(
			__( 'Which level (1–20)?', 'beyond-elysium' )
		);
		if ( ! level ) {
			return;
		}
	...
	return (
		<div className="be-admin be-apr-settings">
			<div className="be-help-heading">
				<h1>{ __( 'Action & Rumor Settings', 'beyond-elysium' ) }</h1>
				<HelpButton helpKey="apr-settings" />
			</div>
			<p>
				{ __(
			...
			{ savedNotice && (
				<p className="be-apr-settings__saved" role="status">
				...
				<>
					...
					{ tab === 'actions' && (
						<div className="be-apr-settings__panel">
							<label>
								{ __(
									'Personal actions per character',
									'beyond-elysium'
								) }
								<input
									type="number"
							...
							<label className="be-apr-settings__checkbox">
								<input
									...
									onChange={ ( e ) =>
										setSettings( {
											...settings,
											carry_unused: e.target.checked,
										} )
									}
								/>
								{ __(
									'Copy Unused Values from Previous Action',
									'beyond-elysium'
								) }
							</label>
							...
							<h2>
								{ __( 'Actions per level', 'beyond-elysium' ) }
							</h2>
							<p className="be-apr-settings__hint">
								{ __(
									'Overrides the default of 2 actions per dot for a specific rating. A rating with no row here uses the default.',
									'beyond-elysium'
								) }
							</p>
							...
							<button type="button" onClick={ addLevel }>
								{ __( '+ Add a level', 'beyond-elysium' ) }
							</button>

							<h2>
								{ __(
									'Backgrounds that grant an action',
									'beyond-elysium'
								) }
							</h2>
							<p className="be-apr-settings__hint">
								{ __(
									'An Influence always grants an action and is shown for reference only. A Background grants one only when checked here.',
									'beyond-elysium'
								) }
							</p>
								...
								{ options.map( ( opt ) => (
									<li key={ opt.name }>
										<label
											className={
												opt.is_influence
													? 'be-apr-settings__checkbox is-disabled'
													: 'be-apr-settings__checkbox'
											}
										>
											<input
												type="checkbox"
												checked={
													opt.is_influence ||
													settings.background_actions.includes(
														opt.name
													)
												}
											...
											{ opt.is_influence &&
												` — ${ __(
													'always granted (Influence)',
													'beyond-elysium'
												) }` }
											<span className="be-apr-settings__stacks">
												{ ' ' }
												({ opt.stacks.join( ', ' ) })
											</span>
										</label>
						...
						<div className="be-apr-settings__panel">
							{ (
								[
									[
										'public_rumors',
										__( 'Public rumors', 'beyond-elysium' ),
									],
									[
										'personal_rumors',
										__(
									...
									[
										'race_rumors',
										__( 'Race rumors', 'beyond-elysium' ),
									],
									[
										'group_rumors',
										__(
											'Group rumors (Clan, Tribe, Kith, Tradition…)',
											'beyond-elysium'
										),
									],
									[
										'subgroup_rumors',
										__(
											'Subgroup rumors (Sect, Auspice, Seeming, Guild…)',
											'beyond-elysium'
										),
									],
									[
									...
									[
										'copy_previous',
										__(
											'Copy previous rumor descriptions',
											'beyond-elysium'
										),
									],
								] as const
								...
								<label
									className="be-apr-settings__checkbox"
									key={ key }
								>
									<input
										type="checkbox"
					...
					<div className="be-admin__form-actions">
						...
						<button
							type="button"
							onClick={ restoreGrapevineDefaults }
						>
							{ __(
