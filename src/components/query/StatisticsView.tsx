...
const STAT_TYPES: { value: StatisticType; label: string }[] = [
	...
	{
		value: 'distinct_distribution',
		label: __( 'Distinct Trait Distribution', 'beyond-elysium' ),
	},
	{
		value: 'specific_distribution',
		label: __( 'Specific Trait Distribution', 'beyond-elysium' ),
	},
...
export function StatisticsView( {
	...
	const [ statType, setStatType ] =
		useState< StatisticType >( 'distribution' );
	...
	function run( e: React.FormEvent ) {
		...
		onRun(
			...
			statType,
			okZero,
			statType === 'specific_distribution' ? trait : undefined
		);
	}
		...
		<div className="be-statistics-view">
			<form className="be-statistics-view__form" onSubmit={ run }>
				<select
					...
					aria-label={ __(
						'Field to generate statistics for',
						'beyond-elysium'
					) }
				>
					<option value="">
						{ __( 'Select a field…', 'beyond-elysium' ) }
					</option>
					{ fields.map( ( f ) => (
				...
				<select
					value={ statType }
					onChange={ ( e ) =>
						setStatType( e.target.value as StatisticType )
					}
					aria-label={ __(
						'Statistic type to run',
						'beyond-elysium'
					) }
				>
					{ STAT_TYPES.map( ( t ) => (
				...
				{ statType === 'specific_distribution' && (
					<input
						type="text"
						placeholder={ __( 'Trait name…', 'beyond-elysium' ) }
						value={ trait }
				...
				{ ( statType === 'distribution' ||
					...
					<label>
						<input
							type="checkbox"
							checked={ okZero }
							onChange={ ( e ) => setOkZero( e.target.checked ) }
						/>
						...
						{ Object.entries( result.buckets )
							...
							.map( ( [ bucket, value ] ) => (
								<li key={ bucket }>
									...
									>
										<span className="be-statistics-view__bucket-label">
											{ bucket }
										</span>
										...
										<span className="be-statistics-view__bucket-value">
											{ value }
										</span>
									</button>
									{ expanded === bucket && (
