/**
 * Type definitions for chronicle factions and their court positions (1.1.0 §3.10, F1/F2).
 */
import type { AudienceRules, AudienceValue } from './plot';

/**
 * `faction_type` is free text server-side (Faction::FACTION_TYPES is a picker suggestion
 * list, not an enforced enum) - this stays `string`, never a strict union, to match.
 */
export type FactionType = string;

/** The picker's suggested vocabulary - matches `Faction::FACTION_TYPES` exactly. */
...
/**
 * The narrower subset a player's own faction proposal may create - matches
 * `Faction::PLAYER_PROPOSABLE_TYPES` exactly.
 */
...
/** A candidate for `POST /factions/{id}/members` - the name-only picker shape. */
...
export interface Position {
	...
	audience: AudienceValue;
	/**
	 * Whether anyone currently holds this position - true even when `character_id` is
	 * withheld from this viewer by `holder_public = false` ("Held - not public").
	 */
	held: boolean;
...
/** The title preset groups (`GET /position-presets`) - preset group label => titles. */
