...
describe( 'draftStorage', () => {
	...
	it( 'round-trips a saved draft back out for the same character', () => {
		...
		expect( loaded?.sheetData ).toEqual( {
	...
	it( "keeps different characters' drafts separate", () => {
		...
		expect( loadDraft( 1 )?.sheetData ).toEqual( {
			virtues: [ { name: 'Conscience', count: 3 } ],
		} );
		expect( loadDraft( 2 )?.sheetData ).toEqual( {
			virtues: [ { name: 'Conviction', count: 4 } ],
		} );
	} );
