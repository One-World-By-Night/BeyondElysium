...
/**
 * `Temper_Display.php` (authoritative) and `displayTemper.ts` (the on-screen sheet's own rendering) must agree.
 */
class TemperDisplayParityTest extends TestCase {
	...
	/**
	 * A pool's temporary value is never clamped down to its permanent value: the excess renders as overflow glyphs past
	 * the permanent track.
	 */
	public function test_temporary_above_permanent_renders_overflow_not_clamped(): void {
		...
		// Not clamped to 3 filled dots: temporary.
