...
/**
 * `working.md`'s real data-quality finding: `vampire-disciplines` seeded "Awakening of the
 * Steel" and "Dur An Ki: Awakening the Steel" as two separate top-level families,
 * byte-identical except for `note` text - two source passes (GVM vs. MET-CSV overlay,
 * Decision 043) that were never reconciled. `Schema::dedupe_awakening_of_the_steel()`
 * removes the bare duplicate and migrates any held pick onto the surviving,
 * more-informative tradition-prefixed name.
 *
...
class AwakeningOfTheSteelDedupeTest extends WP_UnitTestCase {
	...
	/**
	 * The real catalog already seeds `self::BARE` and `self::SURVIVING` (that is the
	 * whole finding this test file covers) - this only adds `self::BARE` back if a prior
	 * dedupe run within the same test already removed it, so every test starts from a
	 * known "both present" state regardless of run order within this file.
	 */
	private function ensure_both_families_present(): void {
		...
		foreach ( $powers as $power ) {
			if ( strcasecmp( (string) ( $power->name ?? '' ), self::BARE ) === 0 ) {
				$has_bare = true;
				break;
			}
		}
		if ( ! $has_bare ) {
			$powers[] = (object) [ 'name' => self::BARE, 'levels' => array_map( 'json_decode', array_map( 'wp_json_encode', $this->levels() ) ) ];
